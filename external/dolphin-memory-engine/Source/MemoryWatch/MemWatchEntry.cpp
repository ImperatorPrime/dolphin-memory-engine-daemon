...
MemWatchEntry::MemWatchEntry(std::string& label, const u32 consoleAddress, const Common::MemType type,
...
MemWatchEntry::MemWatchEntry(MemWatchEntry* entry)
    : m_label(entry->m_label), m_consoleAddress(entry->m_consoleAddress), m_type(entry->m_type),
      ...
      m_length(entry->m_length), m_curActualAddress(entry->m_curActualAddress),
